...
<template>
  <v-toolbar color="#FFFFFF" style="position: fixed; top: 0; left: 0; right: 0; z-index: 999;">
    ...
    <v-tabs fixed-tabs slider-color="#F3B007" class="d-none d-md-flex">
      <v-tab v-for="item in items" :key="item" height="100%" :to="item" class="textItems">
        {{ $t(item) }}
      </v-tab>
    </v-tabs>
    <v-spacer></v-spacer>
    <v-menu offset-y open-on-hover v-if="session" class="d-none d-md-flex">
    ...
    <v-menu offset-y open-on-hover v-else class="d-none d-md-flex">
      <template v-slot:activator="{ isActive, props }">
        <v-btn variant="plain" rounded="0" class="align-self-center me-4 d-none d-md-flex" height="100%" v-bind="props">
          {{ langCurrent }}
          <font-awesome-icon :icon="!isActive ? 'fa-solid fa-chevron-down' : 'fa-solid fa-chevron-up'" />
        </v-btn>
      </template>
    ...
    <v-btn icon @click="redirectLinkedin" class="d-none d-md-flex">
      <font-awesome-icon icon="fa-brands fa-linkedin-in" style="color: #F3B007" />
    </v-btn>
    <v-spacer></v-spacer>
    <v-btn class="d-flex d-md-none" @click="drawer = true">
      <font-awesome-icon icon="fa-solid fa-bars" />
    </v-btn>
  </v-toolbar>
  <v-navigation-drawer v-model="drawer" style="padding-top: 64px;" location="right" class="d-md-none">
    <v-list>
      ...
      <v-list-group value="Users">
        ...
        <v-list-item v-for="lang in langs" :key="lang" :title="lang" :value="lang" @click="changeLang(lang)"
          v-else></v-list-item>
      </v-list-group>
      <v-list-item>
